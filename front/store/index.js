...
export const mutations = {
  ...
  addStudent (state, paylaod) {
    state.students.push(paylaod.student)
  },
  updateStudent (state, paylaod) {
    state.students.forEach((student, index) => {
      if (student.id === paylaod.student.id) {
        state.students.splice(index, 1, paylaod.student)
      }
    })
  },
  removeStudent (state, paylaod) {
    state.students.forEach((student, index) => {
      if (student.id === paylaod.student.id) {
